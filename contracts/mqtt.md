...
# A4 제안 — 승인 대기 중 (미구현)
pcbang/{store_id}/entities/{ha_entity_id}/long_open   # 장시간 개방 감지 (Edge → Backend)
```
...
```
> `entities`: 현재 `string[]` (ha_entity_id 목록).
...
>
> ⚠️ **A4 계획 변경 승인 대기**: `entities` 필드를 `object[]`로 변경하는 제안 진행 중.
> 승인 시 형식 변경 예정: `[{"ha_entity_id": "...", "threshold_minutes": 5}, ...]`
> 변경 전까지 현재 `string[]` 형식 유지.
...
```
> Edge asyncio 타이머 방식으로 구현 예정 (Custom Component 내 `coordinator.py`).
> threshold_minutes는 `monitored_entities` payload에 포함되어 Backend가 전달 (A4 승인 시).
> Backend는 이 토픽 구독 후 AlertEvent 생성 → Frontend WebSocket 브로드캐스트.
> **현재 미구현. Orchestrator 승인 및 `monitored_entities` 계약 변경 후 구현.**
